...
public class ConfigureColorRangefinder extends LinearOpMode {
    ...
    // try
    // run configuration opmode
    // fails
    // configure color sensor
    // error communicating with I2C device
    // plug in sensor
    // error warning goes away
    // run configuration opmode again, nothing happens...
    // wait, ran it a second time, LED blinks twice! Theoretically it's configured
    // now, switch it to a DIGITAL port (or Analog, if you configured it that way)
    // you must change the config to match
    @Override
    public void runOpMode() throws InterruptedException {
        ColorRangefinder crf = new ColorRangefinder(hardwareMap.get(RevColorSensorV3.class, "colorBrushland"));
        waitForStart();
        /* Using this example configuration, you can detect all three sample colors based on which pin is reading true:
    both      --> yellow
    only pin0 --> blue          INTO THE DEEP
    only pin1 --> red
    neither   --> no object
